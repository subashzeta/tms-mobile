...
export default function DriverExpenditureScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <GradientHeader title="My Expenses" subtitle="Your recorded expenses" icon="cash-remove" />
      <FlatList
        data={items} keyExtractor={(item) => item._id}
        contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}
        ListEmptyComponent={<View style={styles.centered}><Text style={styles.emptyText}>No expenses found</Text></View>}
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[colors.primary]} />}
        renderItem={({ item }) => (
          <GradientCard icon={(catIcons[item.category] || 'cash-remove') as any} iconColor={colors.danger}
