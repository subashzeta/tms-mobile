...
export default function DriverForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', err?.response?.data?.message || err?.message || 'Failed to create driver')
    } finally { setLoading(false) }
  }

  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <GradientHeader title="Add Driver" subtitle="Register a new driver" icon="account-plus" />
      <View style={{ flex: 1 }}>
