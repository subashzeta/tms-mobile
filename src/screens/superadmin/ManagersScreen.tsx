...
export default function SuperAdminManagersScreen() {
  const navigation = useNavigation<any>()
  const { data, loading, error, refresh } = useApi<ApiResponse<User[]>>(() => users.getManagers(), [])
  ...
  return (
    <SafeAreaView style={styles.container}>
      <GradientHeader title="Managers" icon="account-tie" />
      <FlatList
        data={items}
        ...
        renderItem={({ item }) => (
          <GradientCard
            ...
            subtitle={item.phone}
            onPress={() => navigation.navigate('SAManagerDetail', { managerId: item._id })}
            stats={[
